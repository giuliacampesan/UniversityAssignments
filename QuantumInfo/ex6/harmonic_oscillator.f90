module harmonic_oscillator
    ...
    function ham_init(N, interval, M,O, t0, T) result(ham)
        ! input arguments
        ...
        do ii=1,N+1
            do jj=1,N+1
                if (ABS(ii-jj)==1) then
                    ...
                    ham(ii,jj) =   (1.0d0/(2.0d0*M)) * (2.0d0/(DX**2)) + (0.5d0*M*O**2) * (L1 + DX*(ii-1) - t0/T)**2
                else
                    ham(ii,jj) =    0.0d0
                end if
            end do
    ...
    subroutine diagonalize_hamiltonian(h, eigv)
        real(8) :: h(:,:), eigv(:)
        integer N, LDZ, INFO
        real(8),  allocatable ::  UD(:), WORK(:)  !, eigvalues(:)
        integer ii
        

        N = size(h, 1)
        LDZ = MAX(1, N)
        !allocate(eigvalues(N)); 
    ...
    program main
        ...
        !filename = 'outfile/x_'//TRIM(ch_Nx)//'_lbound'//TRIM(ch_L1)//'.dat'
        unit=12
        open(unit, file=filename, status='unknown')
        do ii=1, Nx+1
